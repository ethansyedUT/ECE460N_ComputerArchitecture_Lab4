...
void eval_micro_sequencer()
{
     ...
     */
    int microIns[CONTROL_STORE_BITS];
...
void latch_datapath_values()
{
    ...
    if (GetLD_VCB(CURRENT_LATCHES.MICROINSTRUCTION))
    {
        ...
        int val = 0;
        if ((NEXT_LATCHES.MAR - 0x3000 < 0) & ((CURRENT_LATCHES.PSR & 0x8000) >> 15))
        {
        ...
        if (GetUA_EN(CURRENT_LATCHES.MICROINSTRUCTION) & (NEXT_LATCHES.MAR & 1))
        {
            val = val | 1;
        }
